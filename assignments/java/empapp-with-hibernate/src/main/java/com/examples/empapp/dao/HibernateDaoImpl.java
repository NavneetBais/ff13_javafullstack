...
public class HibernateDaoImpl implements EmployeeDao{
	...
	private ArrayList<Employee> getEmployeeCountAgeGreaterThan(int age){
...
//				list = (ArrayList<Employee>) session.createNativeQuery("SELECT * FROM Employee Where age > 25", Employee.class).list();
				...
				list = (ArrayList<Employee>) cr.list();
	...
	private List<Integer> getEmployeeIdsAgeGreaterThan(){
		...
		try {
			session = factory.openSession();
			tnx = session.beginTransaction();
			ArrayList<Employee> empList = getEmployeeCountAgeGreaterThan(30) ;
...
//			}
	...
	public void statisticsHi() {
		// TODO Auto-generated method stub
		System.out.println("No of employees older than thirty years: "+ getEmployeeCountAgeGreaterThan(30).size());
